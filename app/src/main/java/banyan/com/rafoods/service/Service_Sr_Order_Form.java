...
public class Service_Sr_Order_Form extends Service {
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                Cursor cursor = dbManager.Fetch_SR_Order_Form();
                    ...
                    try {
                        ...
                        if (cursor.moveToFirst()) {
                            do {
                                ...
                                String order_form_id = cursor.getString(cursor.getColumnIndex(DatabaseHelper.ORDER_SR_ID));
                                String shop_id = cursor.getString(cursor.getColumnIndex(DatabaseHelper.ORDER_SR_SHOP_ID));
                                String final_order = cursor.getString(cursor.getColumnIndex(DatabaseHelper.ORDER_SR_FINAL_ORDER));
                                String user_id = cursor.getString(cursor.getColumnIndex(DatabaseHelper.ORDER_SR_USER_ID));
                                String user_type = cursor.getString(cursor.getColumnIndex(DatabaseHelper.ORDER_SR_USER_TYPE));
                                String return_type = cursor.getString(cursor.getColumnIndex(DatabaseHelper.ORDER_SR_RETURN_TYPE));
                                String remark = cursor.getString(cursor.getColumnIndex(DatabaseHelper.ORDER_SR_REMARK));

                                Log.i(TAG, "### order_form_id : " + order_form_id);

                                queue = Volley.newRequestQueue(getApplicationContext());
                                // upload order data to server
                                Save_Order_Form(order_form_id, shop_id, final_order, user_id, user_type, return_type, remark);
    ...
    private void Save_Order_Form(final String order_form_id, final String shop_id, final String final_order, final String user_id
            , final String user_type , final String return_type , final String remarks) {

        System.out.println("### Save_Order_Form");
        System.out.println("###  App_Config.url_user_details " + AppConfig.url_add_sales_return);
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    if (success == 1) {
                        ...
                        int result = dbManager.delete_Sr_Order_Form(Long.parseLong(order_form_id));
                        if (result != 0)
